 ...
 * @version 1.0
 */
public abstract class BDD extends AbstractDAO {

	/** The sql example by id. */
	private static String sqlExampleById = "{call findMAPById(?)}";

	private static String sqlAllExamples = "{call findAllExample(?)}";

	private static String sqlExampleByName = "{call findMAPByName(?)}";
	 ...
	 *             the SQL exception
	 */
	public static Blob getExampleById(final int id) throws SQLException {
		...
		if (callStatement.execute()) {
			...
			if (result.first()) {
				blob = result.getBlob(nameColumnIndex);
			}
			result.close();
		}
		return blob;
